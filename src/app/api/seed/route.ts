...
import { createProject, createWorkExperience, createEducation, getAllProjects } from '@/lib/db';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingProjects.length > 0) {
      return NextResponse.json({ message: 'Database already seeded', count: existingProjects.length });
    }
...
export async function GET() {
  ...
  try {
    ...
    const work = getAllProjects(); // Note: we'll import getAllWorkExperience later
